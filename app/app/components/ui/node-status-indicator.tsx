...
export const SpinnerLoadingIndicator = ({ children }: { children: ReactNode }) => {
  return (
    <div className="relative">
      ...
      <div className="absolute inset-0 z-50 rounded-md bg-background/50 backdrop-blur-sm" />
...
const StatusBorder = ({ children, className }: { children: ReactNode; className?: string }) => {
  return (
    <>
      <div
        className={cn(
          "absolute -left-[1px] -top-[1px] h-[calc(100%+2px)] w-[calc(100%+2px)] rounded-md border-2",
          className,
        )}
      />
      {children}
