...
import { useSkillsStore } from "@/contexts/skills-context";
...
export default function IDEPage() {
  ...
  const { skills, selectedSkill, selectSkill, fetchSkills } = useSkillsStore();
  ...
  const handleRunCode = async () => {
    if (editorMode === "skill" && skillEditorManager) {
      console.log("[v0] Running skill:", selectedSkill?.id);
      // Add execution message to chat
      addChatMessage('user', `Executing skill: ${selectedSkill?.name}`);
      await simulateSkillExecution();
    } else {
      console.log("[v0] Running code:", code);
      // Add execution message to chat
      addChatMessage('user', 'Running code...');
      await simulateCodeExecution();
    }
  };

  const addChatMessage = (role: 'user' | 'assistant', content: string, metadata?: any) => {
  ...
  const simulateSkillExecution = async () => {
    ...
    addChatMessage('assistant', 
      ...
      {
        skills_used: ['code-analyzer', 'security-scanner'],
  ...
  const handleSendMessage = async () => {
    ...
    if (userMessage.toLowerCase().includes('help') || userMessage.toLowerCase().includes('how')) {
      response += `Here are some ways I can assist you:\n\n` +
                  `🔧 **Code Analysis** - I can analyze your code for quality and security\n` +
                  `📊 **Data Visualization** - Create charts and graphs from your data\n` +
                  `📚 **Documentation** - Generate API docs and guides\n` +
                  `💬 **Chat Assistant** - Answer questions about your project\n` +
                  `🔍 **Knowledge Base** - Search through project documentation\n\n` +
                  `What would you like to do?`;
    } else if (userMessage.toLowerCase().includes('code') || userMessage.toLowerCase().includes('error')) {
      response += `I can help you with code issues. Let me analyze the current code and provide suggestions. ` +
                  `I notice you're working with ${language} code. Would you like me to run a code analysis?`;
    } else {
      response += `Based on your current project context, here's what I found relevant to your question. ` +
                  `Is there anything specific you'd like me to help you with in the IDE?`;
    }
    ...
    const mockResults = [
      {
        ...
        content: 'Learn how to integrate with our API using authentication tokens...',
        relevance: 0.95,
      ...
      },
      {
        ...
        content: 'Following these best practices will help you write cleaner, more maintainable code...',
        relevance: 0.87,
  ...
  const handleSaveCode = () => {
    if (editorMode === "skill" && skillEditorManager) {
      ...
      addChatMessage('assistant', `💾 **Skill saved successfully!**\n\nAll changes have been preserved.`);
    } else {
      console.log("[v0] Saving code");
      addChatMessage('assistant', `💾 **Code saved!**\n\nYour changes have been saved locally.`);
    }
  };

  const handleOpenSkill = (skillId: string) => {
    const skill = skills.find(s => s.id === skillId);
    if (skill && skillEditorManager) {
      selectSkill(skillId);
      setEditorMode("skill");
      skillEditorManager.loadSkillFiles(skill);
      addChatMessage('assistant', `📦 **Skill loaded:** ${skill.name}\n\nYou can now edit the skill files.`);
    }
  };
